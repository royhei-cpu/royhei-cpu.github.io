...
assert manifest['source'] == origin and manifest['release'] == 26
target.mkdir(parents=True, exist_ok=True)
for name in ['index.html', 'connect.html', 'guest-host.js', 'connect.css', 'connect.js', 'key-input.js', 'voice-check.html', 'voice-check.js']:
...
def download(entry):
    ...
    for attempt in range(3):
        try:
            request = urllib.request.Request(origin + '/' + str(name), headers={'User-Agent': 'Pawlish-GitHub-Release/26'})
